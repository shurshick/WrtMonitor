...
Этот документ описывает установку сервера `wrtmonitor` для тестовой версии `0.1.0-test.6`.
...
1. Откройте релиз:
   [v0.1.0-test.6](https://github.com/shurshick/wrtmonitor/releases/tag/v0.1.0-test.6)

2. Скачайте файл:

   ```text
   wrtmonitor-truenas-0.1.0-test.6.yaml
   ```
...
3. Если пакет GHCR приватный, добавьте в TrueNAS учётные данные для `ghcr.io`.
   ...
   ```text
   ghcr.io/shurshick/wrtmonitor:0.1.0-test.6
   ```
