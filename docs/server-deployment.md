...
Схема базы ведётся через Alembic. Новая пустая PostgreSQL база поднимается миграциями. При обновлении существующие данные сохраняются, а необходимые изменения схемы применяются при старте контейнера.
