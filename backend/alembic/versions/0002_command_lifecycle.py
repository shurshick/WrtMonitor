...
def upgrade() -> None:
    op.add_column("device_commands", sa.Column("picked_at", sa.DateTime(timezone=True), nullable=True))
    op.add_column("device_commands", sa.Column("completed_at", sa.DateTime(timezone=True), nullable=True))
    op.add_column("device_commands", sa.Column("expires_at", sa.DateTime(timezone=True), nullable=True))
    op.add_column("device_commands", sa.Column("retry_count", sa.Integer(), nullable=False, server_default="0"))
    op.add_column("device_commands", sa.Column("last_error", sa.Text(), nullable=True))
    op.add_column("device_commands", sa.Column("source", sa.String(length=40), nullable=False, server_default="api"))
    op.execute("UPDATE device_commands SET expires_at = created_at + interval '5 minutes' WHERE expires_at IS NULL")
