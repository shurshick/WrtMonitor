...
from backend.app.services.commands import create_device_command, expire_old_commands, now_utc
...
def test_telemetry_summary_handles_missing_wifi():
    summary = build_telemetry_summary({"system": {"uptime": 5, "memory": {"total_kb": 2048}}})
...
def test_create_command_has_expiry():
    ...
    db = FakeSession()
    command = create_device_command(db, device_id=uuid4(), command_type="wifi.status", payload={}, created_by=None, source="api")
