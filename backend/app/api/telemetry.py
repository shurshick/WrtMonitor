...
from ..web.routes import TELEMETRY_STALE_SECONDS, cleanup_device_telemetry, current_user, device_from_token, get_user_device_or_404, settings
...
def latest_device_telemetry(device_id: UUID, user: User = Depends(current_user), db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    telemetry = db.scalars(select(DeviceTelemetry).where(DeviceTelemetry.device_id == device_id).order_by(DeviceTelemetry.created_at.desc()).limit(1)).first()
    if not telemetry:
        return {"device_id": str(device_id), "telemetry": None, "created_at": None, "age_seconds": None, "is_stale": False, "source": "agent", "summary": None}
    age_seconds = max(0, int((datetime.now(UTC) - telemetry.created_at).total_seconds()))
    return {"device_id": str(device_id), "telemetry": telemetry.payload, "created_at": telemetry.created_at.isoformat(), "age_seconds": age_seconds, "is_stale": age_seconds > TELEMETRY_STALE_SECONDS, "source": "agent", "summary": build_telemetry_summary(telemetry.payload)}
