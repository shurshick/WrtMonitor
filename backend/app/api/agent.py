...
from ..web.routes import audit, device_from_token
...
def register_agent(payload: AgentRegisterRequest, db: Session = Depends(get_db)) -> dict[str, str]:
    existing = db.scalars(select(Device).where(Device.token_hash == hash_token(payload.device_token))).first()
    if existing:
...
@router.get("/commands")
def poll_commands(authorization: str | None = Header(default=None), db: Session = Depends(get_db)) -> list[dict]:
    ...
    for command in commands:
        command.status, command.updated_at, command.picked_at, command.retry_count = "sent", now, now, command.retry_count + 1
    db.commit()
...
def command_result(command_id: UUID, payload: CommandResultRequest, authorization: str | None = Header(default=None), db: Session = Depends(get_db)) -> dict[str, str]:
    ...
    command.last_error = str(payload.result.get("error")) if payload.result.get("error") else None
