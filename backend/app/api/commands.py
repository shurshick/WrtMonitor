...
from ..web.routes import ALLOWED_COMMANDS, audit, current_user, get_user_device_or_404
...
def list_device_commands(device_id: UUID, limit: int = 20, status: str | None = None, user: User = Depends(current_user), db: Session = Depends(get_db)) -> list[dict[str, Any]]:
    ...
    return [{"id": str(command.id), "command_type": command.command_type, "status": command.status, "source": command.source, "payload": command.payload, "result": command.result, "created_at": iso(command.created_at), "picked_at": iso(command.picked_at), "completed_at": iso(command.completed_at), "expires_at": iso(command.expires_at), "last_error": command.last_error} for command in commands]
